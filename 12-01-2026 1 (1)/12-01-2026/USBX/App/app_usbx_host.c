...
/* Private function prototypes -----------------------------------------------*/
 VOID app_ux_host_thread_entry(ULONG thread_input);
...
UINT MX_USBX_Host_Init()
{
  ...
  TX_BYTE_POOL *byte_pool = (VOID *)&ux_host_app_byte_pool;
  ...
  if (tx_byte_allocate(byte_pool, (VOID **) &HOST_pointer,
  ...
  if (ux_system_initialize(HOST_pointer, USBX_HOST_MEMORY_STACK_SIZE, UX_NULL, 0) != UX_SUCCESS)
  {
...
UINT MX_USBX_Host_DeInit()
{
  ...
  ux_host_stack_hcd_unregister(_ux_system_host_hcd_stm32_name, (ULONG)USB_OTG_HS,
 ...
 __HAL_RCC_USB_OTG_HS_FORCE_RESET();
	 ...
	 if(HOST_pointer)
	 {
		 tx_byte_release(HOST_pointer);
		 HOST_pointer=NULL;
	 }

  tx_byte_pool_delete(&ux_host_app_byte_pool);
  return 0;
}
/**
...
VOID USBX_APP_Host_Init(VOID)
{
  ...
  ux_host_stack_hcd_register(_ux_system_host_hcd_stm32_name,
		  ux_hcd_stm32_initialize, (ULONG)USB_OTG_HS,
                             (ULONG)&hhcd_USB_OTG_HS);
